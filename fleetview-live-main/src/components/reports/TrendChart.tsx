...
export function TrendChart({ showPrev = true }: { showPrev?: boolean }) {
  ...
  const yP = (p: number) => PAD_T + innerH - ((p - 80) / 20) * innerH;
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: 220 }}>
      ...
      {/* on-time line (right axis) */}
      <polyline points={linePts(otp, yP)} fill="none" stroke={GREEN} strokeWidth="2" />
      {otp.map((v, i) => (
        <circle key={i} cx={x(i)} cy={yP(v)} r="2.2" fill={GREEN} />
      ))}
