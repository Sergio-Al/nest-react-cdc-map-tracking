...
│
├── integration-service-nest/          # Microservicio NestJS (Kafka → MySQL)
│   ├── Dockerfile                    # Build multi-stage node:20-alpine
│   ├── package.json
│   └── src/
│       ├── main.ts                   # Bootstrap: HTTP (salud/métricas) en :8090
│       ├── config/configuration.ts   # Configuración por variables de entorno
│       ├── database/database.config.ts # Conexión TypeORM MySQL (synchronize:false)
...
- **Conductor vs. acceso son cosas distintas**: un registro de conductor es operacional y no tiene credenciales. Para que un conductor inicie sesión, un admin/dispatcher aprovisiona un acceso: `POST /api/drivers/:id/login` `{email, password}` crea un usuario `role:'driver'` enlazado vía `driverId` (el tenant se toma del JWT del admin, no del body; un acceso por conductor — `409` si ya existe). `GET /api/drivers` devuelve `hasLogin` por fila; el dashboard muestra la acción "Crear acceso" / la insignia "Tiene acceso" según corresponda. El conductor luego inicia sesión con correo + contraseña + espacio de trabajo; el JWT lleva `driverId` para el acceso restringido por conductor (sus visitas, autoemparejamiento). El dashboard web es para admin/dispatcher — un acceso `role:'driver'` solo ve En Vivo/Historial/Configuración; la experiencia completa del conductor es la futura app compañera.
...
- **SubscriptionsController**: `GET /api/me/entitlements` (flags de funciones para el frontend — el dashboard oculta/deshabilita la UI restringida a partir de esto) y `GET /api/tenant/subscription` (admin).
- **Ciclo de facturación (Stripe)**: `SubscriptionLifecycleService` posee el lado de escritura. Una prueba inversa de 14 días se inicia automáticamente al registrarse el dueño (`AuthService.register`, `role: 'admin'`, idempotente). `POST /api/subscriptions/checkout` abre una sesión de Stripe Checkout (crea el cliente, cupos = nº de conductores activos) para agregar tarjeta y convertir; `POST /api/subscriptions/portal` abre el Billing Portal; `POST /api/subscriptions/trial/start` (admin). Un cron diario degrada las pruebas vencidas no convertidas al plan gratuito Starter. `StripeService` envuelve el SDK; `BillingService` consume el webhook `POST /api/subscriptions/webhook` (público, firma verificada contra el cuerpo crudo) y proyecta `checkout.session.completed`, `customer.subscription.*` e `invoice.paid|payment_failed` sobre `tenant_subscriptions`. Stripe es opcional — con `STRIPE_SECRET_KEY`/`STRIPE_WEBHOOK_SECRET` sin definir, los endpoints de facturación devuelven 503/400. Para operar en vivo además se necesitan Prices reales de Stripe mapeados vía `subscription_plans.stripe_price_id`.
